...
test.describe('User Management Snackbar Notifications', () => {
  ...
  test('Shows snackbar on successful member invitation', async ({ page, request }) => {
    const newUser = generateTestUser('invite-member');
    await request.post(`${API_URL}/auth/signup`, {
      data: { email: newUser.email, password: newUser.password, full_name: newUser.fullName },
    });
  ...
  test('Can invite users with different roles', async ({ page, request }) => {
    ...
    await request.post(`${API_URL}/auth/signup`, {
      data: { email: adminUser.email, password: adminUser.password, full_name: adminUser.fullName },
    });
  ...
  test('Shows snackbar on successful category access update', async ({ page }) => {
    const accessIcon = page.locator('tbody tr').filter({ hasNot: page.locator('text=Owner') }).first().locator('button[title*="access"], button:has(svg)').last();
    ...
    if (await accessIcon.count() > 0) {
      ...
      const categoryCheckbox = page.locator('.MuiDialog-root input[type="checkbox"]').first();
      if (await categoryCheckbox.count() > 0) {
        await categoryCheckbox.click();
      }

      await page.locator('.MuiDialog-root').getByRole('button', { name: 'Save' }).click();

      const snackbar = page.locator('.MuiSnackbar-root');
      await expect(snackbar).toBeVisible();
      await expect(snackbar.locator('.MuiAlert-message')).toContainText('Category access updated successfully');
    }
  });

  test('Shows snackbar on category access update error', async ({ page }) => {
  });

  test('Owner cannot have category access modified', async ({ page }) => {
    const ownerRow = page.locator('tbody tr').filter({ has: page.locator('text=Owner') }).first();

    if (await ownerRow.count() > 0) {
      await expect(ownerRow.locator('text=Owner has access to all categories')).toBeVisible();
  ...
  test('Member list displays correctly', async ({ page }) => {
    ...
    await expect(page.getByRole('columnheader', { name: 'Category Access', exact: true })).toBeVisible();
  });

  test('Role badges display correctly', async ({ page }) => {
    const roleBadges = page.locator('.MuiChip-root').filter({ hasText: /OWNER|ADMIN|MEMBER/ });
    await expect(roleBadges.first()).toBeVisible();
  });

  test('No alerts visible on page - only snackbars', async ({ page, request }) => {
    const testUser = generateTestUser('alert-test');
    await request.post(`${API_URL}/auth/signup`, {
      data: { email: testUser.email, password: testUser.password, full_name: testUser.fullName },
    });
